...
def create_alert(db: Session, alert: schemas.AlertCreate, user_id: uuid.UUID) -> models.Alert:
    # Convert GeoJSON-like Pydantic model to WKT format for GeoAlchemy2
    wkt_location = f"POINT({alert.location.coordinates[0]} {alert.location.coordinates[1]})"
...
def get_review_by_admin_and_alert(db: Session, alert_id: uuid.UUID, admin_id: uuid.UUID):
    """Check if an admin has already voted on a specific alert."""
    return db.query(models.AdminReview).filter(
...
def count_alert_votes(db: Session, alert_id: uuid.UUID):
    approvals = db.query(models.AdminReview).filter(
        models.AdminReview.alert_id == alert_id,
        models.AdminReview.vote == True
    ).count()
    rejections = db.query(models.AdminReview).filter(
        models.AdminReview.alert_id == alert_id,
        models.AdminReview.vote == False
    ).count()
