...
def test_create_alert_success(client: TestClient, db_session: Session, test_user: User, auth_headers):
    """
...
def test_create_alert_with_user_not_in_db(client: TestClient, db_session: Session, auth_headers):
    """
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("type", "invalid_type", "Input should be 'alert', 'news', 'sale', 'help' or 'event'"),
        ...
        ("location", {"type": "Point", "coordinates": [181, 90]}, "longitude must be between -180 and 180"),
    ]
)
def test_create_alert_with_invalid_data(client: TestClient, test_user: User, auth_headers, field, value, error_msg):
    """
...
def test_create_alert_with_missing_required_fields(client: TestClient, test_user: User, auth_headers):
    """
    ...
    # Missing 'description'
    response = client.post("/alerts/", json={"type": "sale", "location": {"type": "Point", "coordinates": [0,0]}, "severity": 1})
    assert response.status_code == 422
    assert "Field required" in str(response.json()["detail"])
    assert "description" in str(response.json()["detail"])

    # Missing 'location'
    response = client.post("/alerts/", json={"description": "test", "type": "sale", "severity": 1})
    assert response.status_code == 422
    assert "Field required" in str(response.json()["detail"])
