...
def test_clerk_webhook_user_created_success(client: TestClient, db_session: Session):
    ...
    response = client.post("/api/v1/webhooks/clerk", content=payload_str, headers=headers)
...
def test_clerk_webhook_idempotency(client: TestClient, db_session: Session, test_user):
    ...
    user_count = db_session.query(User).filter(User.userid == "user_regular_123").count()
    assert user_count == 1
